...
const migrate = async () => {
	const recipes = await pool.query('select * from recipe').then((res) => res.rows.map(migrateRecipe))
	const ingredients = await pool.query('select * from ingredient').then((res) => res.rows.map(({ title }) => title))
